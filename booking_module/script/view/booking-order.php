...
<table border="1" cellpadding="5" width="800">
	<tr>
		...
		<form action="booked.php" method="post">
			<h3>Make booking</h3>
			<p><input checked="checked" name="item" type="radio" value="BookingOrder" /> Booking Order
			<input name="item" type="radio" value="WorkOrder" /> Work Order
			<table style="width: 70%">
				...
				<tr>
					...
					<select id="test123" name="kd_team">
					<option> </option>
      <?php 
      $mySql = "SELECT * FROM team";
      $myQry = mysqli_query($connection,$mySql) or die ("Gagal Query".mysql_error());
      while ($myData = mysqli_fetch_array($myQry)) {
        if ( $dataleaderName == $myData['kd_team']) {
          $cek = " selected";
        } else { $cek='';}
        echo "<option value='$myData[kd_team]' $cek>$myData[nm_team]</option> ";
      }
      ?>
    </select>
    ...
    <tr>
					<td>Team Leader</td>
					<td><input type="text" id="nm_leader" name="nm_leader"> 
					<td><input type="hidden" id="kd_team" >
     </td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
				</tr>
			...
			<input id="from" name="start_day" required="" type="text" /></td>
				...
				<tr>
					<td>&nbsp;</td>
					<td> <select name="start_hour" id="starthour">
...
function draw_calendar($month,$year){
	...
	for($list_day = 1; $list_day <= $days_in_month; $list_day++):
		$calendar.= '<td class="calendar-day">';
			...
			$sql = "SELECT * , team.nm_team FROM $tablename INNER JOIN team ON $tablename.kd_team=team.kd_team WHERE $current_epoch BETWEEN start_day AND end_day";
    		...
    		if (mysqli_num_rows($result) > 0) {
    			...
    			while($row = mysqli_fetch_assoc($result)) {
    				...
    				$calendar .= "<b>" . $row["item"] . "</b><br>ID: " . $row["id"] . "<br>" . $row["name"] . "<br>" . $row["phone"] . "<br>" . $row["nm_team"] . "<br>" . $row["nm_leader"] . "<br>" ;
...
<script type="text/javascript">
  ...
  var json = data,
  obj = JSON.parse(json);
  $('#nm_leader').val(obj.nm_leader);
		$('#kd_team').val(obj.kd_team);
 });
	})
    ...
    success: function(JSONObject) {
      ...
      for (var key in JSONObject) {
        if (JSONObject.hasOwnProperty(key)) {
          peopleHTML += "<tr>";
            peopleHTML += "<td>" + JSONObject[key]["nm_team"] + "</td>";
            peopleHTML += "<td>" + JSONObject[key]["nm_employees"] + "</td>";
												peopleHTML += "<td>" + JSONObject[key]["kd_employees"] + "</td>";
												peopleHTML += "<td>" + JSONObject[key]["nm_barang"] + "</td>";
												peopleHTML += "<td>" + JSONObject[key]["kd_barang"] + "</td>";
          peopleHTML += "</tr>";
        }
      }
